  ...
  <main-container>
    <el-card class="table-card" v-loading="result.loading">
      <template v-slot:header>
        ...
        <package-rule-header :condition.sync="condition"
                            @search="search"
...
import PackageRuleHeader from "../head/PackageRuleHeader";
import TablePagination from "../../common/pagination/TablePagination";
import TableOperator from "../../common/components/TableOperator";
import DialogFooter from "../head/DialogRuleFooter";
import {_filter, _sort} from "@/common/js/utils";
import RuleType from "./RuleType";
import {PACKAGE_RULE_CONFIGS} from "../../common/components/search/search-components";
...
export default {
  ...
  data() {
    return {
      result: {},
      condition: {
        components: PACKAGE_RULE_CONFIGS
      },
      tableData: [],
      currentPage: 1,
      pageSize: 10,
  ...
  methods: {
    ...
    handleDelete(item) {
      this.$alert(this.$t('account.delete_confirm') + item.name + " ？", '', {
        ...
        callback: (action) => {
          if (action === 'confirm') {
            this.result = this.$get("/package/deletePackageRule/" + item.id, () => {
    ...
    //查询列表
    search() {
      let url = "/package/ruleList/" + this.currentPage + "/" + this.pageSize;
    ...
    saveRule (mdObj, type) {
      ...
      let form = '';
      if (type === 'add') {
        url = '/package/addPackageRule';
        form = 'createRuleForm';
      } else if (type === 'edit') {
        url = '/package/updatePackageRule';
        form = 'updateRuleForm';
        if (mdObj.flag == 1) {
    ...
    changeStatus (item) {
      this.result = this.$post('/package/changeStatus', {id: item.id, status: item.status?1:0}, response => {
        if (item.status == 1) {
