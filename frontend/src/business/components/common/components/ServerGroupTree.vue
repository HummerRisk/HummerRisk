<template>
  <div>
    <slot name="header">
      <el-input :placeholder="$t('server.server_search')" v-model="filterText" size="small" :clearable="true"/>
    </slot>
...
export default {
  ...
  props: {
    ...
    allLabel: {
      type: String,
      default() {
        return this.$t("server.all_server");
      }
    },
  },
  watch: {
