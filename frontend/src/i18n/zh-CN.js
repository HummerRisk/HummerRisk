...
export default {
  commons: {
    ...
    cloud_scan_desc: '混合云安全治理服务可供您评估、审计和评价您的云资源配置。持续检测和记录您的云资源配置，并支持您自动依据配置需求评估记录的配置。借助安全治理，您可以查看配置更改以及多云资源之间的关系、深入探究详细的资源配置历史记录并判断您的配置在整体上是否符合内部指南中所指定的配置要求。如此一来，您将能够简化合规性审计、安全性分析、变更管理和操作故障排除。',
  ...
  k8s: {
    ...
    execute_scan: '执行检测',
    result: 'K8s检测结果',
    result_list: 'K8s检测结果列表',
    back_resource: '返回K8s检测结果列表',
    result_details_list: 'K8s检测结果详情列表',
    k8s_note: '注: 使用K8s安全检测前，需在 K8s 集群上安装 tirvy-operator!',
    sync_log: '同步日志',
