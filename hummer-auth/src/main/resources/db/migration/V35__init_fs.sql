...
INSERT INTO `server_rule` (`id`, `name`, `status`, `severity`, `description`, `script`, `parameter`, `last_modified`, `flag`) VALUES ('562fd271-6484-42db-b72b-08d679cb4e03', 'Linux 入侵措施检测', 1, 'MediumRisk', '查看 Linux 入侵检测的措施', 'flag=true\nif [[ $(more /var/log/secure | grep refused|wc -l) -lt 5 ]];then\n    echo \"HummerSuccess: 检测通过，没有异常登录\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: 存在多次登录身份验证失败，请及时处理！\"\nfi', '[]', concat(unix_timestamp(now()), '002'), 1);
