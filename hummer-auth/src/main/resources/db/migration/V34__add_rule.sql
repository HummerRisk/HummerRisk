...
ALTER TABLE `server_result` ADD `is_severity` tinyint    DEFAULT 1 COMMENT '是否有风险';

ALTER TABLE `history_server_task` ADD `is_severity` tinyint    DEFAULT 1 COMMENT '是否有风险';
...
DELETE FROM `server_rule` WHERE `id` = '8f7d6459-a421-4cd3-b1ce-c34a40c237ea';
DELETE FROM `server_rule` WHERE `id` = '77c50b70-4d5c-4231-b484-1cbcd4a7f5fe';
DELETE FROM `server_rule` WHERE `id` = '63b8bb9e-02c5-48ca-a7d4-0eeb93bcfef2';
DELETE FROM `server_rule` WHERE `id` = '691efd98-419d-4643-a001-ed0faaa3defa';
DELETE FROM `server_rule` WHERE `id` = '78637c3b-2206-40f2-9882-a3d29a6d2222';
DELETE FROM `server_rule` WHERE `id` = 'becf4c7c-0976-44ef-9b3e-ba93abf46f71';

UPDATE `server_rule` SET `name` = 'Linux CPU 检测', `status` = 1, `severity` = 'HighRisk', `description` = 'Linux 下查看 CPU 使用率', `script` = 'cpu=`top -b -n5 | fgrep \"Cpu(s)\" | tail -1 | awk -F\'id,\' \'{split($1, vs, \",\"); v=vs[length(vs)]; sub(/\\s+/, \"\", v);sub(/\\s+/, \"\", v); printf \"%d\", 100-v;}\'`\nflag=true\n\nif [[ ${cpu} -lt ${cpuUse}  ]];then\n    echo \"HummerSuccess: 检测通过，CPU资源空闲正常\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: CPU使用率过高，当前使用率 ${cpu}% ,请及时处理！\"\nfi', `parameter` = '[{\"key\":\"cpuUse\",\"name\":\"CPU使用率\",\"defaultValue\":\"80\",\"required\":true}]' WHERE `id` = '73fdb432-62c4-4b08-9682-44dae7e6c312';
UPDATE `server_rule` SET `name` = 'Linux 内存检测', `status` = 1, `severity` = 'HighRisk', `description` = 'Linux 下查看内存使用情况', `script` = 'memory_used=`head -2 /proc/meminfo | awk \'NR==1{t=$2}NR==2{f=$2;print(t-f)*100/t}\'`\nmemory_cache=`head -5 /proc/meminfo | awk \'NR==1{t=$2}NR==5{c=$2;print c*100/t}\'`\nmemory_buffer=`head -4 /proc/meminfo | awk \'NR==1{t=$2}NR==4{d=$2;print d*100/t}\'`\necho \"memory_used: \" $memory_used\necho \"memory_cache: \" $memory_cache\necho \"memory_buffer: \" $memory_buffer\nflag=true\n\nif [[ `echo ${memory_used} > ${memoryUsed}|bc` -eq 1 ]];then\n    echo \"HummerSuccess: 检测通过，ssh服务配置正常\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: 检测到内存使用率超出阈值，请及时处理！\"\nfi', `parameter` = '[{\"key\":\"memoryUsed\",\"name\":\"内存使用率\",\"defaultValue\":\"80\",\"required\":true}]' WHERE `id` = '17d8a321-4eb2-41cc-a466-e29293dc8755';
UPDATE `server_rule` SET `name` = 'Linux 消耗内存进程检测', `status` = 1, `severity` = 'MediumRisk', `description` = '查看 Linux 消耗内存最多的进程', `script` = 'flag=true\nIFS=$\'\\n\\n\'\nfor d in $(ps auxf |sort -nr -k ${taskNum} |head -5|tr -s \" \");do\n  if [ `echo \"$(ps auxf |sort -nr -k 4 |head -5|tr -s \" \"|head -1|cut -d \" \" -f 4|awk -F% \'{print $1}\') > ${memoryUse}\"|bc` -eq 1 ];then\n  memName=`echo $d|awk \'{print $6}\'`\n  echo \"HummerError: 检测内存 ${memName} 使用率超过 ${memoryUse}%，建议观察内存使用情况，或进行扩容\"\n  flag=false\n  break\n  fi\ndone\nif $flag ;then\n   echo \"HummerSuccess: 内存检测正常\"\nfi', `parameter` = '[{\"key\":\"memoryUse\",\"name\":\"内存使用率%\",\"defaultValue\":\"70\",\"required\":true},{\"key\":\"taskNum\",\"name\":\"进程数量\",\"defaultValue\":\"5\",\"required\":true}]' WHERE `id` = '9eacb3a6-6129-44b9-b29a-c07ad449b063';
UPDATE `server_rule` SET `name` = 'Linux CPU 消耗进程检测', `status` = 1, `severity` = 'MediumRisk', `description` = '查看 Linux 系统 CPU消耗最多的进程', `script` = 'flag=true\nIFS=$\'\\n\\n\'\nfor d in $(ps auxf |sort -nr -k 3 |head -${taskNum}|tr -s \" \");do\n  if [ `echo \"$(ps auxf |sort -nr -k 3 |head -5|tr -s \" \"|head -1|cut -d \" \" -f 4|awk -F% \'{print $1}\') > ${cpuUse}\"|bc` -eq 1 ];then\n  cpuName=`echo $d|awk \'{print $6}\'`\n  echo \"HummerError: 检测CPU ${cpuName} 使用率超过 ${cpuUse}%，建议观察CPU使用情况，或进行扩容\"\n  flag=false\n  break\n  fi\ndone\nif $flag ;then\n   echo \"HummerSuccess: CPU检测正常\"\nfi', `parameter` = '[{\"key\":\"taskNum\",\"name\":\"进程数量\",\"defaultValue\":\"5\",\"required\":true},{\"key\":\"cpuUse\",\"name\":\"CPU使用率\",\"defaultValue\":\"80\",\"required\":true}]' WHERE `id` = 'a1c9a8d3-97d2-4556-abdb-5d77ea46bad5';
UPDATE `server_rule` SET `name` = 'Linux 磁盘使用率检测', `status` = 1, `severity` = 'LowRisk', `description` = '检测 Linux 操作系统的磁盘使用率是否超过阈值', `script` = 'flag=true\nIFS=$\'\\n\\n\'\nfor d in $(df |grep /|tr -s \" \");do\n  if [[ $(echo $d|cut -d \" \" -f 5|awk -F% \'{print $1}\') -gt ${diskUse} ]];then\n  diskName=`echo $d|awk \'{print $6}\'`\n  echo \"HummerError: 检测磁盘 ${diskName} 使用率超过 ${diskUse}%，建议观察磁盘使用情况，或进行扩容\"\n  flag=false\n  break\n  fi\ndone\nif $flag ;then\n   echo \"HummerSuccess: 磁盘空间检测正常\"\nfi', `parameter` = '[{\"key\":\"diskUse\",\"name\":\"磁盘使用率，百分百1-100\",\"defaultValue\":\"80\",\"required\":true}]' WHERE `id` = '6f378375-bbe2-47f7-85fc-4a8e8c8193c6';
UPDATE `server_rule` SET `name` = 'Linux 开机自启项检测', `status` = 1, `severity` = 'LowRisk', `description` = '查看 Linux 开机自启项', `script` = 'systemctl list-unit-files\nflag=true\nif [[ $(systemctl list-unit-files|grep ${service}|awk \'{print $2}\') =~ \"enabled\" ]];then\n    echo \"HummerSuccess: 检测通过，${service} 服务已经是开机自动启动\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: 检测失败，${service} 服务未开机自动启动，请及时处理！\"\nfi', `parameter` = '[{\"key\":\"service\",\"name\":\"服务名\",\"defaultValue\":\"sshd.service\",\"required\":true}]' WHERE `id` = '26cf695f-fca6-4097-bf02-f9cb30f79c0d';
UPDATE `server_rule` SET `name` = 'Linux 主机入侵检测', `status` = 1, `severity` = 'MediumRisk', `description` = '检查 Linux 是否安装了主机入侵检测软件', `script` = 'flag=true\nfor checkFile in `find / -iname ${value} -print`;do\nif [ -f  ${checkFile} ];then\n    echo \"HummerError: 检测到异常文件 ${checkFile},请及时处理！\"\n    flag=false\n    break\nfi\ndone\nif $flag ;then\n   echo \"HummerSuccess: 未检测到异常文件\"\nfi', `parameter` = '[{\"defaultValue\":\"ddgs*\",\"name\":\"主机入侵检测软件\",\"key\":\"value\",\"required\":true}]' WHERE `id` = '11c940a7-24e0-4f9d-a9a9-9e6811b12ad6';
UPDATE `server_rule` SET `name` = 'Linux audit 运行状态检测', `status` = 1, `severity` = 'MediumRisk', `description` = '查看 Linux audit 运行状态', `script` = 'auditctl -s\nflag=true\nif [[ $(auditctl -s|grep enable|awk \'{print $2}\') != \"1\" ]];then\n    echo \"HummerError: auditctl规则审计功能未开启，请及时处理！\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n   echo \"HummerSuccess: auditctl规则审计功能已开启\"\nfi', `parameter` = '[]' WHERE `id` = '161b15b6-b5e3-420d-b0fc-de172553df74';
UPDATE `server_rule` SET `name` = 'Linux 审计日志检测', `status` = 1, `severity` = 'MediumRisk', `description` = '查看 Linux 是否开启审计日志服务', `script` = 'flag=true\nif [[ $(service auditd status|grep active|awk \'{ print $2 }\') =~ \"active\" ]];then\n    echo \"HummerSuccess: auditctl规则审计功能已开启\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: auditctl规则审计功能未开启，请及时处理！\"\nfi', `parameter` = '[]' WHERE `id` = '18365786-1f24-4a42-b5a5-291b9274205a';
UPDATE `server_rule` SET `name` = 'Linux 审计规则检测', `status` = 1, `severity` = 'LowRisk', `description` = '查看 Linux 服务器审计规则', `script` = 'flag=true\nif [[ $(more /etc/audit/audit.rules|wc -l) -gt 3 ]];then\n    echo \"HummerSuccess: auditctl 审计规则已设置\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: auditctl 审计规则未设置，请及时处理！\"\nfi', `parameter` = '[]' WHERE `id` = '1feaf989-836f-4f90-975d-8caac4b18692';
UPDATE `server_rule` SET `name` = 'Linux 入侵检测', `status` = 1, `severity` = 'MediumRisk', `description` = '查看 Linux 入侵检测的措施', `script` = 'flag=true\nif [[ $(more /var/log/secure | grep refused|wc -l) -lt 5 ]];then\n    echo \"HummerSuccess: 检测通过，没有异常登录\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: 存在多次登录身份验证失败，请及时处理！\"\nfi', `parameter` = '[]' WHERE `id` = '562fd271-6484-42db-b72b-08d679cb4e03';
UPDATE `server_rule` SET `name` = 'Linux audit 规则检测', `status` = 1, `severity` = 'LowRisk', `description` = '查看服务器审计规则', `script` = 'flag=true\nif [[ $(auditctl -l|wc -l) -gt 1 ]];then\n    echo \"HummerSuccess: auditctl 审计规则已设置\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: auditctl 审计规则未设置，请及时处理！\"\nfi', `parameter` = '[]' WHERE `id` = 'bb389762-8756-405d-b37c-fc789d1cc9cc';
UPDATE `server_rule` SET `name` = 'Linux 资源限定检测', `status` = 1, `severity` = 'MediumRisk', `description` = '查看 Linux 下是否对资源进行了限定', `script` = 'flag=true\nif [[ $(more /etc/security/limits.conf|grep -v \'^#\'|grep -v \'^$\'|wc -l) -gt 10 ]];then\n    echo \"HummerSuccess: 检测通过，已经配置资源限定规则\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: 未配置资源限定规则，请及时处理！\"\nfi', `parameter` = '[]' WHERE `id` = '87ecea8f-3ba1-48d0-8fcd-b3d74e1dae60';
UPDATE `server_rule` SET `name` = 'Linux 日志检测', `status` = 1, `severity` = 'MediumRisk', `description` = '查看日志是否运行', `script` = 'flag=true\nif [[ $(systemctl status rsyslog|grep Active|awk \'{print $2}\') =~ \"active\" ]];then\n    echo \"HummerSuccess: 检测通过，rsyslog 日志服务已经启动\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: 检测失败，rsyslog 日志服务未启动，请及时处理！\"\nfi', `parameter` = '[]' WHERE `id` = 'f45cc8e2-867f-4e97-8998-ce7f0c070d2d';
UPDATE `server_rule` SET `name` = 'Linux SSH 暴力登入检测', `status` = 1, `severity` = 'HighRisk', `description` = '查看 Linux 操作系统是否存在 SSH 暴力登入信息', `script` = 'flag=true\nif more /var/log/secure | grep \"Failed\" > /dev/null;then\n    more /var/log/secure|awk \'/Failed/{print $(NF-3)}\'|sort|uniq -c|awk \'{print \"HummerError: 登入失败的IP和尝试次数: \"$2\"=\"$1\"次---------[需调整]\";}\'\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerSuccess: 检测通过，无ssh暴力登入信息\"\nfi', `parameter` = '[]' WHERE `id` = '5ca75854-b03c-4e9f-9ec7-4bf77f16d94d';
UPDATE `server_rule` SET `name` = 'Linux 系统密码复杂度检测', `status` = 1, `severity` = 'LowRisk', `description` = '查看 Linux 系统密码复杂度要求是否设置', `script` = 'grep -i \"^password.*requisite.*pam_cracklib.so\" /etc/pam.d/system-auth > /dev/null\nif [ $? == 0 ];then\n	echo \"HummerSuccess: 密码复杂度:已设置\"\nelse\n	grep -i \"pam_pwquality\\.so\" /etc/pam.d/system-auth > /dev/null\nif [ $? == 0 ];then\n	echo \"HummerSuccess: 密码复杂度:已设置\"\nelse\n	echo \"HummerError: 密码复杂度:未设置,请加固密码--------[需调整]\"\nfi\nfi', `parameter` = '[]' WHERE `id` = '15370452-3ce3-429c-8743-4a344baddb26';
UPDATE `server_rule` SET `name` = 'Linux SSH 远程密码登录检测', `status` = 1, `severity` = 'LowRisk', `description` = '检测 Linux 系统是否运行 SSH 远程密码登录', `script` = 'flag=true\nif [[ $(grep -i \"^PasswordAuthentication\" /etc/ssh/sshd_config|awk \'{print $2}\') =~ no ]];then\n    echo \"HummerSuccess: 检测通过，SSH服务配置正常\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: 当前设置运行使用密码进行远程登录，安全性较低，建议使用SSH密钥登录，请及时处理！\"\nfi', `parameter` = '[]' WHERE `id` = '7f742d41-d91d-4597-8462-25a89f8841e0';
UPDATE `server_rule` SET `name` = 'Linux 僵尸进程检测', `status` = 1, `severity` = 'MediumRisk', `description` = '查看 Linux 僵尸进程', `script` = 'flag=true\nif if [[ $(ps aux | awk \'{ print $8 \" \" $2 }\' | grep -w Z) == \"\" ]];then\n    echo \"HummerSuccess: 检测通过，无 Linux 僵尸进程\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: 检测到僵尸进程 $(ps aux | awk \'{ print $8 \" \" $2 }\' | grep -w Z)\"\nfi', `parameter` = '[]' WHERE `id` = 'c497a676-12ec-41ed-8944-eb32cc95b8a4';
UPDATE `server_rule` SET `name` = 'Linux PID 检测', `status` = 1, `severity` = 'LowRisk', `description` = '查看 Linux PID资源使用情况', `script` = 'use_num=$(ps -eLf | wc -l)\nsys_num=$(cat /proc/sys/kernel/pid_max)\nif [[ $use_num -gt $sys_num ]];then\n	echo \"HummerError: PID 耗尽，将会影响程序正常运行\"\nelse\n  	echo \"HummerSuccess: PID 资源正常\"\nfi', `parameter` = '[]' WHERE `id` = 'f00222ed-4a39-43be-b2a8-5b5b72529296';
UPDATE `server_rule` SET `name` = 'Linux 内存溢出检测', `status` = 1, `severity` = 'MediumRisk', `description` = '查看 Linux 系统进程是否存在内存溢出', `script` = 'flag=true\nif [[ $(grep \"Out of memory\" /var/log/messages) == \"\" ]];then\n    echo \"HummerSuccess: 检测通过,没有发现有内存溢出情况\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: 存在内存溢出，请及时处理！\"\n	echo \"$(grep \"Out of memory\" /var/log/messages)\"\nfi', `parameter` = '[]' WHERE `id` = 'fdf46bee-0ae7-416f-85c8-b17c8dfa08ba';
UPDATE `server_rule` SET `name` = 'Linux 防火墙检测', `status` = 1, `severity` = 'HighRisk', `description` = 'Linux 下查看防火墙是否开启，以及防火墙规则', `script` = 'flag=true\nif [[ $(service firewalld status|grep active|awk \'{ print $2 }\') =~ \"active\" ]];then\n    echo \"HummerSuccess: firewalld 防火墙功能已开启\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: firewalld 功能未开启，请及时处理！\"\nfi', `parameter` = '[]' WHERE `id` = '7a9cfc62-3c78-4f08-8210-d9bbfe47980c';
UPDATE `server_rule` SET `name` = 'Linux 密码过期时间检测', `status` = 1, `severity` = 'LowRisk', `description` = '查看 Linux 密码过期的时间', `script` = 'CurrentPassMaxDays=$(cat /etc/login.defs |grep -v \'^$\'|grep -v \'^#\'|grep PASS_MAX_DAYS|awk \'{print $2}\')\nflag=true\n\nif [[ ${CurrentPassMaxDays} -lt ${PassMaxDays}  ]];then\n    echo \"HummerSuccess: 检测通过，当前密码过期时间为 ${CurrentPassMaxDays}天 ，密码过期时间检测正常\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: 未设置密码过期时间，当前密码默认过期时间为 ${CurrentPassMaxDays}天 ,为避免密码泄露造成的损失，请及时处理！\"\nfi', `parameter` = '[{\"key\":\"PassMaxDays\",\"name\":\"密码过期天数\",\"defaultValue\":\"90\",\"required\":true}]' WHERE `id` = '0cfdd71d-f4c0-42b0-a8a7-af8760640c8e';
UPDATE `server_rule` SET `name` = 'Linux 操作系统类型检测', `status` = 1, `severity` = 'LowRisk', `description` = '查看 Linux 检测操作系统类型', `script` = 'OS=\"$(uname)\"\ncase $OS in\n  \"Linux\")\n    echo \"HummerSuccess: 当前系统为 Linux\";;\n  \"FreeBSD\")\n    echo \"HummerSuccess: 当前系统为 FreeBSD\";;\n  \"WindowsNT\")\n    echo \"HummerSuccess: 当前系统为 WindowsNT\";;\n  \"Darwin\")\n    echo \"HummerSuccess: 当前系统为 Darwin\";;\n  \"SunOS\")\n    echo \"HummerSuccess: 当前系统为 SunOS\";;\n  \"AIX\")\n    echo \"HummerSuccess: 当前系统为 AIX\";;\n  *)\n    echo \"HummerError: 未知的操作系统\";;\nesac', `parameter` = '[]' WHERE `id` = 'ba903623-9d68-4939-ba39-fa34628c2d1d';
UPDATE `server_rule` SET `name` = 'Linux 高危端口检测', `status` = 1, `severity` = 'MediumRisk', `description` = 'Linux 下查看高风险端口', `script` = 'result=$(netstat -tunlp|awk \'{print $4}\'|grep \":${servicePort}\")\nflag=true\nif [[ -z $result ]];then\n    echo \"HummerSuccess: 无高风险端口\"\n    flag=false\n	exit 0\nfi\nif $flag ;then\n    echo \"HummerError: 存在高风险端口 ${servicePort}，请及时处理！ \"\nfi', `parameter` = '[{\"key\":\"servicePort\",\"name\":\"服务端口\",\"defaultValue\":\"80\",\"required\":true}]' WHERE `id` = '10e326fb-8a12-4e4b-9f02-e2ea0e3c7e82';
