    ...
    <el-submenu index="2" popper-class="submenu">
      <template v-slot:title>
        <span class="account-name" :title="currentAccount">
          {{ $t('account.cloud_account') }}: {{ currentAccount }} | {{ $t('commons.last_scan_time') }}: {{ currentAccount }}
        </span>
      </template>
      <search-project-list @cloudAccountSwitch="cloudAccountSwitch" @selectAccount="selectAccount"/>
...
<script>
import SearchProjectList from "@/business/components/common/head/SearchProjectList";
