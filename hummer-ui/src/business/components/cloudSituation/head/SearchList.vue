<template>
  <div v-loading="result.loading" style="width: 300px;">
    ...
    <div v-else style="height: 150px;overflow: auto">
      <el-menu-item :key="i.id" v-for="i in items" @click="change(i.id, i.name)">
        <template slot="title">
          <div class="title">
  ...
  methods: {
    async init () {
      await this.$get(allListUrl, response => {
        ...
        this.items.unshift({id: 'all', name: this.$t('rule.all'), pluginIcon: 'all.png'});
        this.searchArray = response.data;
        if (this.items.length > 0) {
          this.change(this.items[0].id, this.items[0].name);
        }
      });
    },
    query(queryString) {
    ...
    change(accountId, accountName) {
        this.currentAccountId = accountId;
        this.accountName = accountName;
        this.$emit("selectAccount", accountId, this.accountName);
    },
  }
}
</script>
