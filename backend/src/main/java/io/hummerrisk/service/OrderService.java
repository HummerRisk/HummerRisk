...
import io.hummerrisk.base.mapper.ext.ExtResourceMapper;
...
import io.hummerrisk.dto.TaskCopyDTO;
import io.hummerrisk.dto.TaskDTO;
import io.hummerrisk.dto.TaskItemLogDTO;
...
public class OrderService {
    ...
    public TaskDTO getTaskDetail(String taskId) {
        TaskDTO taskDetail = extTaskMapper.getTaskDetail(taskId);
        return taskDetail;
    }
    ...
    public TaskCopyDTO copy(String taskId) {
        ...
        taskItemList.stream().forEach(item -> {
            if (!set.contains(item.getAccountId())) set.add(item.getAccountId());
        });
